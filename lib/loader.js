        ...
        return Model.findOne(findOptions).then(function(instance) {
            ...
            return Model
                ...
                .save(saveOptions).then(function(instance) {
                    if (instance) {
                        if (fixture.handle) {
                            if (!(Model.name in self.reference)) {
                                self.reference[Model.name] = {};
                            }

                            var instanceKeys = {};
                            for (const key of Object.keys(Model.primaryKeys)) {
                                instanceKeys[key] = instance.get(key);
                            }

                            self.reference[Model.name][fixture.handle] = instanceKeys;
                        }
